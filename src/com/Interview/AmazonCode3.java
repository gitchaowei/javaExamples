...
class Result {
    ...
    public static List<List<String>> searchSuggestions(List<String> repository, String customerQuery) {
        ...
        int wordin = customerQuery.length();
        if (wordin < 2) return null;

        //String subStr = customerQuery.substring(0, i);
        List <List<String>> suggLists = new ArrayList<>();
        Collections.sort(repository);

        for (int iw = 2; iw<= wordin; iw++) {

            List <String> suggList = new ArrayList<>();
            String subWord = customerQuery.substring(0, iw);
            int count=0;
            for (String string : repository) {
                if (string.startsWith(subWord)) {

                    suggList.add(string);
                    count++;
                    if (count == 3) {  // require to list maximum 3 suggestions
                        break;
                    }
                }
            }
            suggLists.add(suggList);
        }
        return suggLists;
    }
}
